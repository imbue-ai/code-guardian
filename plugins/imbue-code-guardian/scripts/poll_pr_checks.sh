...
#
# Usage: ./poll_pr_checks.sh [--timeout <seconds>] [--interval <seconds>] <branch_name_or_pr_number>
#
# Output:
...
export STOP_HOOK_SCRIPT_NAME="poll_pr_checks"
...
while true; do
    ...
    if ! CHECK_OUTPUT=$(gh pr checks "$PR_NUMBER" 2>&1); then
        # Distinguish between no checks and failed checks
    ...
    # exit code while keeping the "0" output.
