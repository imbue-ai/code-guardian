...
4. **Gate-skip detection** -- if we're on the base branch, there are no changes vs base, or only `.md` files changed, the hook passes without further checks. Runs before the PR check so these sessions don't trip the "no PR" requirement.
