...
- **Hard error on misconfiguration.** A listed dir that is missing, is not a distinct git repo, or lacks its own `.reviewer/settings.json` blocks the hook -- so it can never silently go unreviewed.
...
- **Unified review gates.** There is one `/autofix` and one `/verify-architecture` per stop cycle covering all dirs -- not one per dir. Each keeps its own dir-local markers, so a gate is satisfied only when every changed dir has a current marker. When reviewing one dir, the agent is given a relevance-filtered, git-grounded summary of related changes in the other dirs (shared interfaces/APIs/contracts) so cross-repo coupling is accounted for.
