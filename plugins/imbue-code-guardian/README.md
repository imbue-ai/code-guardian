...
- **Gitignore the generated reviewer state.** Each reviewed repo must gitignore `.reviewer/outputs/` and `.reviewer/logs/` (the hook writes there every run) and `.reviewer/settings.local.json` (the `reviewer-*` toggle skills write it); otherwise the uncommitted-changes check will trip. `.reviewer/settings.json` is committed.
