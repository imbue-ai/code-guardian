...
Usage:
    python generate-from-vet.py --vet-repo /path/to/vet
    VET_REPO=/path/to/vet python generate-from-vet.py
    python generate-from-vet.py --vet-repo /path/to/vet --check
"""
...
BRANCH_OUTPUT_FORMAT = textwrap.dedent("""\
    ## Output Format
...
def generate_branch_markdown(vet_modules) -> str:
    """Generate verify-branch.md: batched commit + correctness guides."""
    IssueCode = vet_modules["IssueCode"]
...
def generate_conversation_markdown(vet_modules) -> str:
    """Generate verify-conversation.md: conversation history guides."""
    ISSUE_CODES_FOR_CONVERSATION_HISTORY_CHECK = vet_modules["ISSUE_CODES_FOR_CONVERSATION_HISTORY_CHECK"]
    ISSUE_IDENTIFICATION_GUIDES_BY_ISSUE_CODE = vet_modules["ISSUE_IDENTIFICATION_GUIDES_BY_ISSUE_CODE"]

    guides = [ISSUE_IDENTIFICATION_GUIDES_BY_ISSUE_CODE[code]
              for code in ISSUE_CODES_FOR_CONVERSATION_HISTORY_CHECK]
...
def load_vet(vet_repo: Path) -> dict:
    ...
    # Add vet repo to sys.path so we can import from it.
...
def main() -> None:
    ...
    parser.add_argument(
        "--vet-repo",
        type=Path,
        default=os.environ.get("VET_REPO"),
    ...
    if not args.vet_repo:
        parser.error("--vet-repo or VET_REPO env var is required")
    vet_repo = Path(args.vet_repo).resolve()
    ...
    if args.check:
        ...
        for mode in modes_to_run:
            ...
            if not path.exists():
                print(f"MISSING {path.relative_to(PLUGIN_ROOT)}", file=sys.stderr)
                ok = False
            elif path.read_text() != expected:
                print(f"STALE   {path.relative_to(PLUGIN_ROOT)}", file=sys.stderr)
                ok = False
            else:
                print(f"OK      {path.relative_to(PLUGIN_ROOT)}", file=sys.stderr)
        if ok:
    ...
    for mode in modes_to_run:
        ...
        print(f"Wrote {path.relative_to(PLUGIN_ROOT)}", file=sys.stderr)
