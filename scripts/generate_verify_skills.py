...
These are the DEFAULT categories shipped with the imbue-code-guardian plugin.
...
BRANCH_CATEGORIES_PATH = (
    REPO_ROOT / "plugins" / "imbue-code-guardian" / "agents" / "categories" / "code-issue-categories.md"
)
CONVERSATION_CATEGORIES_PATH = (
    REPO_ROOT / "plugins" / "imbue-code-guardian" / "agents" / "categories" / "conversation-issue-categories.md"
)
...
def main() -> None:
    ...
    if args.check and not ok:
        print(
            "Vet defaults are stale. Run 'python scripts/generate_verify_skills.py' to update.",
            file=sys.stderr,
        )
        raise SystemExit(1)
