...
- **Retargeting:** The gate skills and their sub-agents are modified to operate on an explicit set of target repos (`git -C <dir>`, dir-local markers/config) instead of hardcoding the session CWD. The single review pass iterates the dirs internally, spawning its per-repo sub-agents (`validate-diff`, `verify-and-fix`, `analyze-architecture`) with full cross-repo context in scope.
- **Cross-repo reasoning:** Because one agent reviews all dirs, shared surfaces (e.g. an interface in A consumed by B) are handled natively: when fixing B it focuses on the **relevance-filtered** slice of A's changes it depends on, grounding via `git -C`, and calibrates how much extra grounding to ask its sub-agent for based on its confidence in the relationship.
...
- **Cross-repo reasoning inside the single review pass (autofix + architecture):**
  - One agent reviews all dirs, gathering each dir's changes via `git -C <dir> diff` (source of truth, not memory).
  - When working a given dir, it uses a **relevance-filtered** view of the *other* dirs' changes — focusing on the cross-repo surfaces that dir depends on/interacts with (shared interfaces, APIs, contracts) and de-emphasizing unrelated churn.
  - It calibrates how much extra grounding to ask a sub-agent to do based on its confidence in the cross-dir relationship: unsure → briefly orient itself, then instruct the sub-agent to ground further; confident → tell it the sub-agent needn't explore much.
...
- `autofix`, `verify-architecture`: become **single, cross-cutting** commands. Each accepts the *set* of target dirs (those with changes), iterates them internally, and writes a dir-local marker for each. Replace CWD-bound frontmatter (`!`git ...``, `${GIT_BASE_BRANCH:-main}`) with per-dir resolution.
- Their sub-agents (`validate-diff`, `verify-and-fix`, `analyze-architecture`): accept a target-directory parameter and thread it through all git/read/write ops and marker paths.
- Add cross-repo guidance: while working one dir, use a relevance-filtered, `git -C`-grounded view of the *other* dirs' changes; calibrate a sub-agent's grounding effort by the agent's confidence in the cross-dir relationship.
